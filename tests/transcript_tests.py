...
class TestCompareTwoTranscripts(unittest.TestCase):
    ...
    def test_alternate_start_site(self):
        matching_exons, diffevents =\
                compare_two_transcripts('t1', 'total_diff_start',
                        self.transcript_dict)
        
        self.assertEqual(len(diffevents), 1)
        alt_start = diffevents.filter('AS')[0]
        self.assertEqual(alt_start.cigar1, [(0,5), (3, 44)])
    ...
    def test_compare_transcripts_skipped_exon_simple(self):
        ...
        se = diffevents.filter('SE')[0]
        self.assertEqual(len(diffevents), 1)
        self.assertEqual(se.tid, ('t1', 't2'))
