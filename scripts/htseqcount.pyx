...
def creat_mapping(gff_file, feat_type='exon'):
   features = HTSeq.GenomicArrayOfSets( "auto", stranded != "no" )
   counts = {}

   # Try to open samfile to fail early in case it is not there

   gff = open( gff_file , 'rU')
   cdef int i
   i = 0
   try:
       for f in gff:
...
def count_reads_in_features( bam_filename, gff_filename, stranded,
